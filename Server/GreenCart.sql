...
BEGIN
    ...
    -- Set confirmation timestamp directly on the new row
    :NEW.confirmed_date := SYSTIMESTAMP;
END;
/
...
BEGIN
    ...
    -- Check if customer already confirmed and handle accordingly
    DECLARE
    ...
    BEGIN
        ...
        -- Note: We no longer update the order status here as it's handled by the trigger
        -- The trigger will automatically update the order status when both parties confirm
